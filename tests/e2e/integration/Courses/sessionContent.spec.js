...
describe('Sessions content', () => {
  ...
  it('Courses_CreateSessionContent_CreateWithAllContentsTypes', function () {
    ...
    cy.clock().invoke('setSystemTime', Cypress.dayjs('2023/09/11', 'YYYY/MM/DD').toDate().getTime()) // To put after login to make it works
    // Click on session
    ...
    cy.get('.edit-course-modal').should('be.visible').within(() => {
      ...
      // Set description
      cy.get('.ck-editor')
      cy.type_ckeditor(sessionContentToCreate[0].content)
    ...
    cy.get('[data-test="additionalText"]').eq(1)
    cy.type_ckeditor(sessionContentToCreate[0].additionalText)
    // Add cy.tick beacause cy.clock()
    cy.tick(1000)
    ...
    cy.get('.session-details').within(() => {
      ...
      getSessionContentWithSupport(sessionContentToCreate[0]).should('be.visible')
    })
  })
