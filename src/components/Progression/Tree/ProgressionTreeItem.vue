<template>
  <div
    class="dropzone"
    :class="{'visible': draggingItem}"
    @dragleave="cancelActive"
    @dragover="setActive"
    @drop="dropFile"
  />
  <div
    class="tree-item theme-hover-light-background-color"
    :class="{'selected': isSelected, shrinked: draggingItem }"
    :title="item.name"
    :draggable="true"
    @click="selectItem"
    @dragstart="onDragStart"
    @dragend="onDragEnd"
  >
    <div class="icon">
      <img
        v-if="item.isHomework"
        class="item-type-icon"
        src="@assets/devoir.svg"
        :alt="$t('homework')"
        :title="$t('homework')"
      >
      <img
        v-else
        class="item-type-icon"
        src="@assets/seance.svg"
        :alt="$t('session')"
        :title="$t('session')"
      >
    </div>

    <span>{{ item.name }}</span>
  </div>
  <div
    v-if="isLast"
    class="dropzone"
    :class="{'visible': draggingItem }"
    @dragleave="cancelActive"
    @dragover="setActive"
    @drop="dropFile"
  />
</template>

<script>

export default {
  name: 'ProgressionTreeItem',
  props: {
    index: {
      type: Number,
      required: true
    },
    isLast: {
      type: Boolean,
      default: false
    },
    item: {
      type: Object,
      required: true
    },
    isParentSectionSelected: {
      type: Boolean,
      required: true
    }
  },
  data () {
    return {
      isActive: false
    }
  },
  computed: {
    currentItem () {
      return this.$store.state.progression.currentItem
    },
    draggingItem () {
      return this.$store.state.progression.draggedItem
    },
    isSelected () {
      // Item is selected when itself is selected or its parent section/subsection
      return (this.currentItem && this.currentItem.itemId === this.item.itemId) ||
              this.isParentSectionSelected
    }
  },
  methods: {
    selectItem () {
      this.$store.dispatch('progression/getItemContents', this.item)
      this.$store.dispatch('progression/setCurrentItem', this.item)
    },
    onDragStart (e) {
      this.$store.commit('progression/setDraggedItem', this.item)
      const dragItem = {
        index: this.index,
        isSection: false,
        item: this.item
      }

      e.dataTransfer.setData('dragItem', JSON.stringify(dragItem))
    },
    onDragEnd () {
      this.$store.commit('progression/setDraggedItem', undefined)
    },
    setActive (e) {
      e.preventDefault()
      const elem = JSON.parse(e.dataTransfer.getData('dragItem'))

      if (!elem.isSection && elem.item.itemId !== this.item.itemId) {
        e.target.classList.add('drag-over')
      }
    },
    cancelActive (e) {
      e.target.classList.remove('drag-over')
    },
    dropFile (e) {
      const elem = JSON.parse(e.dataTransfer.getData('dragItem'))

      if (!elem.isSection && elem.item.itemId !== this.item.itemId) {
        const itemCopy = { ...elem.item }
        itemCopy.order = (!this.isLast) ? this.index + 1 : this.index + 2
        itemCopy.folderId = this.item.folderId
        this.$store.dispatch('progression/updateItem', itemCopy)
      }
    }
  }
}
</script>

<style lang="scss" scoped>
.tree-item {
  display: flex;
  align-items: center;
  transition: padding-top .3s;
  transition: padding-bottom .3s;
  padding-top: 4px;
  padding-bottom: 4px;
  border-top-right-radius: 15px;
  border-bottom-right-radius: 15px;

  &.shrinked {
    padding-top: 0;
    padding-bottom: 0;
  }

  &.selected {
    color: #306CD3;
  }
}

.dropzone {
  width: 100%;
  height: 0;
  transition: height .3s;

  &.visible {
    height: 15px;
  }

  &.drag-over {
    background-color: lightgrey;
    border: 2px dashed lightslategrey;
    height: 30px;
  }
}

.icon {
  display: flex;
  align-items: center;
  justify-content: center;
  margin-right: 3px;
  margin-left: -3px;

  img {
    width: 22px;
    height: 22px;
  }
}

span {
  flex: 1;
  overflow-x: hidden;
  white-space: nowrap;
  text-overflow: ellipsis;
  font-size: 0.75rem;
}
</style>

<i18n locale="fr">
{
  "session": "Séance",
  "homework": "Devoir"
}
</i18n>
